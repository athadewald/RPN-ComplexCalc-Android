...
% Für die Schriftart times zu verwenden
\usepackage{mathptmx}
...
\title{Ein UPN Rechner für reelle und komplexe Zahlen}
...
\end{figure} 
So erhält man die Werte in dem oberen Bild: \\
...
Komplexe Zahlen bestehen aus zwei Komponenten, dem Realteil und dem Imaginärteil. Das Rechnen ist so ähnlich wie mit binomischen Formeln.
\begin{align*}
(a+b)^2 &= a^2 + 2ab + b^2 \qquad \text{binomische Formel} \\
(a+ib)^2 &= a^2 + i2ab - b^2 = a^2 - b^2 + i2ab \qquad \text{komplexe Zahl}
\end{align*}
Das $-b^2$ entsteht durch das Quadrieren der imaginären Einheit $i$.
\begin{align*}
i^{2} = -1
\end{align*}
Komplexe Zahlen können auf zwei Arten dargestellt werden.
\begin{itemize}
...
Der Realteil 3 muss im Y-Feld und der Imaginärteil 5 im X-Feld stehen .

\subsection{Trigonometrische Funktionen}
Wenn man sich die komplexe Zahlenebene anschaut, sieht man ein rechtwinkliges Dreieck. Es gibt folgende Zusammenhänge für den
...
\end{align*}
Möchte man den Sinus-, Cosinus- und Tangenswert von $\varphi = 30 ^\circ$ berechnen erreicht man das über die Eingabe \\

\qquad 1 \fbox{Enter} 30 \fbox{cplx} \\

mit der Taschenrechnereinstellung auf Pol und Deg.

Stellt man jetzt auf Rect um, erhält man den Sinus- und Cosinuswert. Das ergibt sich aus der 'Eulerschen Formel'.\\ 

Die vollständige Eingabe mit der Einstellung am Rechner auf Pol und Deg:
...
Der letzte Wert in der Tabelle wäre der $\tan(30 ^\circ) = 0.5774$.\\
Hieraus den Winkel wieder zurückrechnen (arctan) mit der Einstellung am Rechner auf Rect und Deg:\\
...
Rechnen mit großen Zahlen. In einer Sendung aus der amerikanischen Comic-Serie 'Die Simpsons' hatte
...
\end{tabular} 
\end{table}
Die linke und die rechte Seite der Gleichung ist identisch. Die Betätigung der \fbox{prec} Taste zeigt aber, dass der letzte
Fermatsche Satz durch diese Gleichung nicht widerlegt wird. Das X- und das Y-Feld werden durch das Drücken dieser Toggle-Taste 
mit einer höheren Genauigkeit angezeigt.
\newpage

\section{Berechnung von Widerstandsschaltungen}

\subsection{Schaltung mit Wirkwiderständen}
...
\subsection{Schaltung mit komplexen Widerständen}

Es soll der Erstatzwiderstand und der Strom berechnet werden (Rect und Deg ist gewählt).
...
Die Eingabe der Werte in den Rechner (Deg und Rect gewählt) zeigt die folgende Tabelle: \\
...
\subsection{Schaltung mit Wirk- und Blindwiderständen}

Es soll der Erstatzwiderstand berechnet werden (Rect ist gewählt).
